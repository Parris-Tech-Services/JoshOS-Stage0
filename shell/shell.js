...
var JoshOS = (function () {
  ...
  var state = {
    ...
    windows: [],     // live window records
    zTop: 100,
    seq: 0,
    focused: null
  };

  /* ---------- persistence (best effort; file:// may refuse) ---------- */

  function save(key, value) {
    try { localStorage.setItem('joshos.' + key, JSON.stringify(value)); } catch (e) { /* no-op */ }
  }
  ...
  // An app declares what it is and how to fill a window body. Nothing else.
  // { id, name, glyph, width, height, mount(bodyEl, win) }
  ...
  function openApp(id) {
    ...
    // One window per app for now. Re-opening focuses the existing one.
  ...
  // Snap zones, Windows-style: left half, right half, top = maximise.
