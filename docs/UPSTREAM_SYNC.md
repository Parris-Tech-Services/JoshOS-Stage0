# JoshOS ↔ JoshOS-Stage0 sync policy

`joshuaparris-max/JoshOS` is the canonical Josh OS integration repository.

`Parris-Tech-Services/JoshOS-Stage0` is an extracted Stage-0 product/ISO workspace.

Because parts of this repository were copied from JoshOS, unmanaged duplication will create contradictory behaviour and documentation. This file defines how to avoid that.
...
JoshOS owns the content. JoshOS-Stage0 mirrors it.
...
|---|---|---|---|
| `design/tokens.json` | JoshOS | exact-match CI | none unless ADR says otherwise |
| generated token CSS | generated | regenerate | output formatting only |
| `shell/` | JoshOS product concepts | selective promote/sync | experiments allowed temporarily |
| `scripts/build-iso.sh` | JoshOS product ISO | deliberate comparison | repo metadata/extraction packaging |
| product docs | JoshOS | links/summaries | JoshOS-Stage0-specific notes only |
...
1. fetch a pinned or current JoshOS revision;
...
2. port the smallest coherent change to JoshOS;
...
```text
UPSTREAM_REPO=joshuaparris-max/JoshOS
UPSTREAM_COMMIT=<sha>
...
The Stage-0 ISO build scripts have already diverged. JoshOS recently removed a VirtualBox guest package from its ArchISO package list to avoid a package conflict, while this extraction needs to be checked separately.
...
Archive it once the canonical product workflow in JoshOS is faster and clearer than maintaining a second live-image repo.
