...
# Overlay files that do not belong to the upstream ArchISO profile.
cp -a "$ROOT/iso/overlay/." "$GENERATED_PROFILE/airootfs/"

# The browser prototype itself is the Stage 0 graphical shell.
...
# Boot directly to the graphical target and enable the display manager.
...
# Keep the upstream bootloader configuration, but make the visible entries ours.
while IFS= read -r -d '' cfg; do
  sed -i \
    -e 's/Arch Linux install medium/Josh OS live/g' \
    -e 's/Arch Linux/Josh OS/g' \
    "$cfg"
